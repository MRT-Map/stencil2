...
pub type ComponentQuery<'w, 's, 'a> = (
    Query<'w, 's, (Entity, &'a mut PlaComponent<EditorCoords>), With<SelectedComponent>>,
    Commands<'w, 's>,
    Res<'w, Skin>,
    ResMut<'w, PrevNamespaceUsed>,
    EventWriter<'w, Action>,
);

pub fn component_ui(ui: &mut egui::Ui, query: ComponentQuery) {
    let (mut selected, mut commands, skin, mut prev_namespace_used, mut actions) = query;
