...
use crate::{
    ...
    ui::{popup::Popup, tilemap::settings::TileSettings},
};
        ...
        if matches!(event.downcast_ref(), Some(TileSettingsAct::Open)) {
            popup.send(Popup::new(
                ...
                |state, ui, ew, shown| {
                    ...
                    ui.checkbox(&mut tile_settings.clear_cache_on_startup, "Clear tile cache on startup");
                    ui.separator();

                    ui.add(egui::Slider::new(&mut tile_settings.max_get_requests, 1..=1000)
